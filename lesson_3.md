...
### 1、配置环境
- 从官方环境复制运行 InternLM 的基础环境，命名为 InternLM2_Huixiangdou，在命令行模式下运行：`studio-conda -o internlm-base -t InternLM2_Huixiangdou`
- 在本地查看环境: `conda env list`
- 激活对应环境： `conda activate InternLM2_Huixiangdou`

### 2、下载文件
- 下载模型文件，选用 InternLM2-Chat-7B 作为基础模型
```
# 创建模型文件夹
...
git checkout b9bc427 # 将仓库的HEAD指针移动到指定的提交
...
```
上面的操作是为了将当前工作目录切换到 "/root/huixiangdou"，备份一个名为 "good_questions.json" 的文件，然后创建一个新的 "good_questions.json" 文件，其中包含一系列关于 "mmpose" 和 "huixiangdou" 的问题。
...
说明：
```
echo '[...]' > /root/huixiangdou/resource/good_questions.json

echo 是一个命令，用于在终端输出文本。

[...] 是一个包含多个问题的JSON格式字符串。

> 是重定向操作符，用于将命令的输出重定向到一个文件。如果文件已存在，它将被覆盖；如果文件不存在，它将被创建。

./test_queries.json 是目标文件路径，将 echo 命令输出的内容写入到这个文件中。
```
